...
function AddHolidayForm({ onAdd }) {
 ...
 const handleSubmit = async (e) => {
  ...
  try {
    ...
    const newHoliday = res.data.holiday || res.data;
    ...
    if (err.response?.status === 400) {
      window.alert(err.response.data.error || "Holiday already exists.");
    } else {
      window.alert("Failed to add holiday. Please try again.");
    }
  } finally {
    setLoading(false);
  }
